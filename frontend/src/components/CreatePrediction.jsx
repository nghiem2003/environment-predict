...
const CreateNewPrediction = () => {
  ...
  const [activeTab, setActiveTab] = useState('single'); // Default to 'single'
  ...
  const handleInputChange = (event) => {
    ...
    // Validate and parse the input as a float
    setInputs({
      ...inputs,
      [name]: value ? parseFloat(value) : 0, // Use parseFloat to convert input to a number
    });
  };
  ...
  const handleSubmitSingle = async (e) => {
    ...
    // Ensure all inputs are valid numbers before submission
    for (let key in inputs) {
      if (isNaN(inputs[key]) || inputs[key] === '') {
        alert(`Invalid value for ${key}. Please provide a valid number.`);
        return;
      }
    }
